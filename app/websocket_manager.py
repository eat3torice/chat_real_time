...
class SimpleWebSocketManager:
    ...
    async def connect(self, user_id: int, websocket):
        was_offline = user_id not in self.connections or len(self.connections[user_id]) == 0
    ...
    async def broadcast_user_status(self, user_id: int, is_online: bool):
        ...
        try:
            ...
            db.close()
            print(f"📡 Broadcasted {user_id} status ({'online' if is_online else 'offline'}) to {len(friend_ids)} friends")
    ...
    async def send_to_conversation(self, conversation_id: int, message: dict):
        ...
        if not user_ids:
            print(f"❌ No members found for conversation {conversation_id}, broadcasting to all connected users")
    ...
    async def send_to_user(self, user_id: int, message: dict):
        ...
        if connections:
            ...
            if sent_count > 0:
                print(f"✅ Sent message to user {user_id}: {message['type']} ({sent_count} connections)")
