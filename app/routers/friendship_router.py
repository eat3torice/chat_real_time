...
def get_friends(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> Any:
    """
    ...
    for friendship in friendships:
        ...
        friend_id = friendship.receiver_id if friendship.requester_id == current_user.id else friendship.requester_id
...
def get_friend_requests(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> Any:
    """
...
def send_friend_request(
    ...
    existing = db.query(Friendship).filter(
        ((Friendship.requester_id == current_user.id) & (Friendship.receiver_id == target_user.id)) |
        ((Friendship.requester_id == target_user.id) & (Friendship.receiver_id == current_user.id))
    ).first()
