...
from app.chat.services import get_or_create_direct_conversation, create_message, get_conversation_member_ids
...
async def send_message_to_conversation(
    ...
    try:
        # Create the message
        msg_dict = create_message(db, payload.conversation_id, current_user.id, payload.content)
        ...
        try:
            ...
            await websocket_manager.send_to_conversation(payload.conversation_id, message_event)
...
def send_message(payload: MessageCreate, current_user=Depends(get_user_by_token), db: Session = Depends(get_db)) -> Any:
    """
