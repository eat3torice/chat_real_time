    ...
    for conv in conversations:
        ...
        result.append(
            ConversationOut(
                ...
                if getattr(conv, "type", None) == "direct"
                else None,
...
def get_conversation(
    ...
    return ConversationOut(
        ...
        if getattr(conversation, "type", None) == "direct"
        else None,
        member_ids=member_ids,
    )
...
def create_conversation(
    ...
    try:
        if conversation_data.type == "direct":
            ...
            if (
                not conversation_data.member_user_ids
                or len(conversation_data.member_user_ids) != 1
            ):
                raise HTTPException(
                    status_code=400,
            ...
            conversation = Conversation(
                name=None, type="direct"  # Direct conversations don't have names
            )
            db.add(conversation)
            db.commit()
            ...
            member1 = ConversationMember(
                conversation_id=conversation.id, user_id=current_user.id, role="member"
            )
            ...
            conv = create_group(
                db,
                current_user.id,
                conversation_data.name or "Group",
                conversation_data.member_user_ids or [],
            )
            member_ids = get_conversation_member_ids(db, conv.id)
            # Only direct conversations have private_pair_key
            return ConversationOut(
                id=conv.id,
                name=conv.name,
                type=conv.type,
...
def create_group_endpoint(
    ...
    try:
        ...
        # Create conversation
        conversation = Conversation(name=payload.name or "Group", type="group")
        db.add(conversation)
        db.commit()
        db.refresh(conversation)
...
def leave_group_endpoint(
    ...
    if not removed:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Not a group member"
        )
...
def transfer_admin_endpoint(
    ...
    try:
        ...
        if not current_member or current_member.role != "admin":
            raise HTTPException(
                status_code=403,
                detail="Only conversation admin can transfer admin role",
            )
        ...
        if not conversation or conversation.type != "group":
            raise HTTPException(
                status_code=400, detail="Can only transfer admin in group conversations"
            )
        ...
        if not new_admin_member:
            raise HTTPException(
                status_code=404, detail="New admin is not a member of this conversation"
            )
        ...
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(
            status_code=500, detail=f"Failed to transfer admin: {str(e)}"
        )
...
def kick_member(
    ...
    if not current_member or current_member.role != "admin":
        raise HTTPException(
            status_code=403, detail="Only conversation admin can kick members"
        )
    ...
    if not target_member:
        raise HTTPException(
            status_code=404, detail="Member not found in this conversation"
        )
    ...
    try:
        ...
        event = {
            ...
            "kicked_by": {"id": current_user.id, "username": current_user.username},
        }
    ...
    return {
        "status": "success",
        "message": f"Đã loại {kicked_user.username if kicked_user else 'thành viên'} khỏi cuộc trò chuyện",
    }
...
def add_member_to_conversation(
    ...
    if conversation.type == "direct":
        raise HTTPException(
            status_code=403, detail="Cannot add members to direct conversations"
        )
    ...
    if not allow_member_add and current_member.role != "admin":
        raise HTTPException(
            status_code=403, detail="Only conversation admin can add members"
        )
    ...
    try:
        ...
        event = {
            "type": "member_added",
            "conversation_id": conversation_id,
            "new_user": {"id": user_id, "username": new_user.username},
            "added_by": {"id": current_user.id, "username": current_user.username},
        }
...
def update_conversation_settings(
    ...
    if not current_member or current_member.role != "admin":
        raise HTTPException(
            status_code=403, detail="Only conversation admin can update settings"
        )
    ...
    return {
        "status": "success",
        "message": "Đã cập nhật cài đặt cuộc trò chuyện",
        "settings": settings,
    }
