...
def create_group(
    ...
    """
    conv = Conversation(name=name, type="group")
    db.add(conv)
    db.flush()  # populate conv.id
    ...
    members.append(
        ConversationMember(
            conversation_id=conv.id, user_id=int(creator_id), role="admin"
        )
    )
    ...
    if initial_member_ids:
        for uid in set(initial_member_ids):
            ...
            members.append(
                ConversationMember(
                    conversation_id=conv.id, user_id=int(uid), role="member"
                )
            )
    db.add_all(members)
    db.commit()
...
def add_member_to_group(
    db: Session, conversation_id: int, username_or_email: str
    ...
    member = ConversationMember(
        conversation_id=conversation_id, user_id=user.id, role="member"
    )
    db.add(member)
    db.commit()
    db.refresh(member)
